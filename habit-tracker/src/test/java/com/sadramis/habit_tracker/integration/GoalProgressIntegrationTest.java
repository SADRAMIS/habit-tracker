...
class GoalProgressIntegrationTest extends BaseIntegrationTest {
    ...
    private Long createGoal(String token, String title, double targetValue) {
        ...
        goalRequest.setDeadline(Instant.now().plus(30, java.time.temporal.ChronoUnit.DAYS));
       ...
       /* if (response.getStatusCode() != HttpStatus.CREATED) {
            // Получаем ответ как строку, чтобы увидеть JSON с ошибкой
            ResponseEntity<String> errorEntity = restTemplate.exchange("/api/v1/goals", HttpMethod.POST, new HttpEntity<>(goalRequest, headers), String.class);
            System.out.println("=== CREATE GOAL ERROR ===");
            System.out.println("Status: " + errorEntity.getStatusCode());
            System.out.println("Body: " + errorEntity.getBody());
        }*/
